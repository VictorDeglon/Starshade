...
function advanceToNextLevel() {
  ...
  const powerUp = StarshadeEconomy.getEquippedPowerUp();
  if (coinsEarned > 0 && powerUp && powerUp.effect === "coinBoost") {
...
// -------------------------------------------------------------
// Settings, the Level Map, Achievements, the Shop, and Contact all open as
// overlays *on top of* whichever root screen was showing — the pause menu
// mid-run, or the main menu — rather than navigating to a separate page.
// Every one of those overlays' own scripts is also loaded here (each
// wrapped in its own IIFE — see their top comments) and drives this exact
// same markup in place instead. Besides keeping the game's state intact
// the way a full navigation never could mid-run, this keeps game.js's own
// `audio` element alive and playing throughout (see docs/architecture.md)
// — there's only one page in the whole app now, so nothing ever tears it
// down. Even "Main Menu" (what used to be "Quit to Menu", a real
// navigation) is just another overlay swap now.
...
function drawStarLayer() {
  ...
  const alpha = Math.max(0.22, smoothstep(0.05, 0.7, sceneProgress));
...
function startSlingshotAim(e) {
  if (!playerOnSlingshot || isPaused || isFading || isPortalSucking) return;
